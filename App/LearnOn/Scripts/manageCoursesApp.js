///<reference path="./../typings/globals/core-js/index.d.ts"/>
///<reference path="TypeLite.Net4.d.ts"/>
"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var core_2 = require('@angular/core');
var http_1 = require('@angular/http');
var platform_browser_1 = require('@angular/platform-browser');
var platform_browser_dynamic_1 = require('@angular/platform-browser-dynamic');
var ManageCoursesComponent = (function () {
    function ManageCoursesComponent(_http) {
        this._http = _http;
        this.title = 'ASP.NET MVC 5 with Angular 2';
        this.courses = [];
        this.getCourses();
    }
    ManageCoursesComponent.prototype.getCourses = function () {
        var _this = this;
        return this._http.get('/odata/Courses')
            .subscribe(function (value) {
            _this.courses = value.json().value;
        });
    };
    ManageCoursesComponent = __decorate([
        core_1.Component({
            selector: 'manage-courses',
            template: "    \n    <h2>My favorite skill is: {{myskills}}</h2>\n    <p>Skill:</p>\n    <ul>\n      <li *ngFor=\"let c of courses\">\n        {{ c.CourseId }}\n      </li>\n    </ul>\n  "
        }), 
        __metadata('design:paramtypes', [http_1.Http])
    ], ManageCoursesComponent);
    return ManageCoursesComponent;
}());
exports.ManageCoursesComponent = ManageCoursesComponent;
var ManageCoursesModule = (function () {
    function ManageCoursesModule() {
    }
    ManageCoursesModule = __decorate([
        core_2.NgModule({
            imports: [platform_browser_1.BrowserModule, http_1.HttpModule],
            declarations: [ManageCoursesComponent],
            bootstrap: [ManageCoursesComponent]
        }), 
        __metadata('design:paramtypes', [])
    ], ManageCoursesModule);
    return ManageCoursesModule;
}());
exports.ManageCoursesModule = ManageCoursesModule;
var platform = platform_browser_dynamic_1.platformBrowserDynamic();
platform.bootstrapModule(ManageCoursesModule);
//# sourceMappingURL=manageCoursesApp.js.map